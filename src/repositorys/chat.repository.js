...
class ChatRepository {
    ...
    selectUserChatRooms = async (connection, mem_no) => {
        const selectQuery =`
            SELECT message.content , user.mem_name, message.chat_room_no
                case
            ...
            FROM message
            INNER JOIN (
                SELECT max(message_no) as last_message_no , chat_room_no, content, reciver_no
                from message
            ...
            inner JOIN member as user on user.mem_no = last_message.reciver_no
        `;
